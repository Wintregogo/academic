...
| 子模块 | 功能描述 | 关键技术点 |
|--------|--------|-----------|
| **ArXiv Fetcher** | 从 arXiv API 获取最新/指定论文元数据 | - 使用 `arxiv` Python 包<br>- 支持按关键词、作者、时间范围查询<br>- 自动下载 PDF 到本地缓存目录 |
| **PDF Parser** | 从 PDF 中提取可读文本 | - **双后端策略**：<br>  • 主路径：调用 Grobid REST API (`/api/processFulltextDocument`)<br>  • 备用路径：`pdfplumber` 提取纯文本<br>- **智能缓存**：<br>  • 基于文件路径+修改时间生成 MD5 key<br>  • Grobid 响应缓存为 `.xml`，pdfplumber 结果缓存为 `.json`<br>- 依赖 `lxml` 确保 XML 解析稳定 |
| **LLM Analyzer** | 调用大模型生成结构化洞察 | - 使用 Jinja2 模板构建 prompt<br>- 支持多后端（Qwen, OpenAI, Ollama）<br>- 输出强制 JSON Schema（便于前端解析）<br>- 自动截断长文本（≤12k 字符）以控制成本 |
| **Cache Manager** | 统一管理各类缓存（PDF、解析结果、LLM 响应） | - 所有缓存存放于 `./cache/` 目录<br>- 缓存键包含文件指纹，确保一致性<br>- 可选：未来支持 LRU 或 TTL 过期策略 |
| **Web UI (Streamlit)** | 提供交互式分析界面 | - 动态加载论文卡片<br>- 支持“使用 Grobid”开关<br>- 实时显示 LLM 分析结果<br>- 错误友好提示（如 Grobid 未启动） |
